...
const Album = () => {
  const { sol, data, favorites } = useAppSelector(selectAlbum);
  const dispatch = useAppDispatch();
  const [page, setPage] = useState<"photos" | "favorites">("photos");
  const changePageHandler = (page: "photos" | "favorites") => setPage(page);

  useEffect(() => {
    if (!data.find((solData) => solData.sol === sol)) {
      dispatch(getSolData(sol));
    }
  }, [data, dispatch, sol]);
    ...
    <>
      <header className={styles.header}>
        <button
          className={cn(styles.button, {
            [styles.button_active]: page === "photos",
          })}
          onClick={() => changePageHandler("photos")}
        >
          PHOTOS
        </button>
        <button
          className={cn(styles.button, {
            [styles.button_active]: page === "favorites",
          })}
      ...
      <section className={styles.content}>
        {page === "photos" && (
          <>
            <header>
              <label htmlFor="sol">Set Soul:</label>
              <input
                type="number"
